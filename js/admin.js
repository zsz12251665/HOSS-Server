...
function SubmitForm(e)
{
	...
	if (document.querySelector('input#Download').checked)
	{
		...
		if (homeworkList[document.querySelector('select#Select').selectedIndex - 1].count == 0)
		{
	...
	{
		form.set('Title', homeworkList[document.querySelector('select#Select').selectedIndex - 1].title);
		form.set('Directory', homeworkList[document.querySelector('select#Select').selectedIndex - 1].directory);
		form.set('Deadline', homeworkList[document.querySelector('select#Select').selectedIndex - 1].deadline);
	}
...
homeworkList.forEach(homework => document.querySelector('select#Select').innerHTML += '<option value="' + homework.directory + '">' + homework.title + ' (' + homework.count + ')</option>');
// Add event listeners
